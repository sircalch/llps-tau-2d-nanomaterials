...
def build_official_manuscript():
    ...
    if os.path.exists(csv_sobol):
        ...
        st_dG_th      = float(df_sobol.loc["dG_ads", "ST_theta"])
        st_conf_dG_th = float(df_sobol.loc["dG_ads", "ST_conf_theta"])
        st_eta_th     = float(df_sobol.loc["eta_eff", "ST_theta"])
        st_conf_eta_th= float(df_sobol.loc["eta_eff", "ST_conf_theta"])
        st_beta_th    = float(df_sobol.loc["beta", "ST_theta"])
        st_conf_beta_th=float(df_sobol.loc["beta", "ST_conf_theta"])
    else:
        ...
        st_dG_th, st_conf_dG_th = 0.67, 0.08
        st_eta_th, st_conf_eta_th = 0.29, 0.04
        st_beta_th, st_conf_beta_th = 0.21, 0.03
    ...
    r_abst = p_abs.add_run(
        ...
        "and kinetic framework that combines Flory-Huggins-Voorn-Overbeek (FH-VO) polymer theory with Langmuir interfacial "
        ...
        "Solution Temperature (LCST) turbidity onset of Tau K18 (15.3 °C at 100 µM), the model shows that 2D "
        ...
        "shifts the apparent cloud point to 29.4 °C at a_s = 1.0×10⁻⁴ nm⁻¹, dissolving condensates at room and sub-physiological "
        "temperatures and depleting ~60% of the free monomer at 37 °C. In contrast, a moderate-affinity Ti3C2Tx MXene-like scenario "
    ...
    body("Figure 2a displays the phase density contrast Δφ_tilde = φ_tilde_dense - φ_tilde_dilute across ionic strength [NaCl] (50–450 mM) and temperature (15–50 °C). Electrostatic screening follows Voorn-Overbeek / Debye-Hückel scaling (-α_DH (I/I_0)^(3/2)) [11,23]: moderate ionic strength preserves condensate stability, whereas elevated salt screens the electrostatic contribution and narrows the two-phase coexistence gap.")
    ...
    body("We state the model's limitations and domain of validity explicitly. (i) This work is deliberately formulated as a predictive theoretical study: bulk phase separation is parameterized against published Tau K18 turbidity measurements [5], whereas the nanomaterial-dependent adsorption, wetting, and kinetic outputs are presented as prospective, falsifiable model-scenario predictions rather than as direct experimental validation. (ii) Tau K18 is an experimentally well-characterized four-repeat (R1-R4) construct that contains the principal amyloidogenic hexapeptide motifs (VQIINK in R2, VQIVYK in R3); extrapolation to full-length Tau (Tau-441), which carries large charged projection domains and a distinct regulatory phosphorylation landscape, will require independent parameterization. (iii) φ_tilde is an effective coarse-grained order parameter representing lattice occupancy rather than atomistic coordinates. (iv) The solvent is treated implicitly, without explicit conformational dynamics. (v) The adsorption isotherm is idealized as non-cooperative Langmuir binding. (vi) Borophene and MXene are represented as idealized, stable 2D nanosheets, without explicit chemical oxidation or restacking kinetics. (vii) Local surface-termination micro-heterogeneity is not resolved. Future work combining all-atom molecular dynamics with continuum phase-field modeling would provide atomistic resolution of the 2D-interface-induced conformational landscape.")
    ...
    body("where χ(T) = χ_c + β (T - Tc_K) encodes LCST thermal sensitivity, and the critical parameters (φ_c = 0.247, χ_c = 0.872) are solved numerically from f''(φ_c) = 0 and f'''(φ_c) = 0. Model parameters are summarized in Table 1.")
    ...
    t1_data = [
        ...
        ["Mass Overlap Conc.", "c*_mass", "141.2", "g/L", "Overlap mass concentration: 3 MW / (4 pi N_A Rh³)"],
    ...
    body("where K_deg = exp(-ΔG_ads / RT). The single unknown φ_tilde_free is obtained by fixed-point (Picard) iteration of this mass balance to a convergence tolerance of 10⁻¹². (Brent's method is used separately, in Section 3.6, to locate the apparent cloud-point temperature.)")
    ...
    body("where η_eff = 0.20×10⁻³ represents the phenomenological coarse-grained interfacial coupling factor (scaling with the ratio of interfacial capillary width to droplet radius, ξ / R ~ 10⁻³). The resulting contact angles are therefore conditional on η_eff. Young's equation then gives:")
    eq("cos(θ_c) = Δγ_s / γ_LL")
    body("The 95% confidence intervals in Figure 3b were evaluated via Monte Carlo error propagation (N_MC = 500 draws, seed = 42) sampling Gaussian distributions for ΔG_ads (standard deviation 0.25 kcal/mol) and η_eff (standard deviation 0.02×10⁻³).")
